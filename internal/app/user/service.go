...
func (service *userService) Login(ctx context.Context, user entities.User) error {
	...
	if err != nil {
		err = exceptions.NewUnauthorizedException(fmt.Sprintf("passwords for user: %s don't match", userFound.Username))
		service.logs.Warn(str.ErrorConcat(err, serviceName, "Login"))
		return err
	}
